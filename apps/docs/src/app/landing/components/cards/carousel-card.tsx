...
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious } from 'vibe-ui'
  ...
  return (
    <Card className="w-full">
      <CardHeader className="pb-4">
        ...
        <CardDescription className="text-xs">Swipe through the highlights.</CardDescription>
      </CardHeader>
      <CardContent>
              ...
              <CarouselItem key={index}>
                <div className="p-1">
                  <div className="flex aspect-[4/3] items-center justify-center rounded-xl bg-muted/50 p-6 border border-border/40">
                    <span className="text-4xl font-semibold text-muted-foreground">{index + 1}</span>
                  </div>
                </div>
              </CarouselItem>
            ))}
