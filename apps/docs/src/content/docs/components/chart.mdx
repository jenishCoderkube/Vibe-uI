...
Vibe UI's Chart component is built as a flexible, responsive container wrapper (`ChartContainer`) around Recharts primitives, styled to match the theme color configuration.

### Responsive Theme Mappings

- **Dynamic Theme Accent Sync**: Chart colors are linked directly to CSS variables `--chart-1` through `--chart-5`. By default, `--chart-1` and `--chart-2` are mapped to `var(--primary)` and `var(--primary) / 0.4`, allowing all charts to dynamically update their accents whenever a new color preset (such as Crimson, Purple, or Green) is selected in the customizer.
- **Adaptive Labels & Fills**: Circular gauges, grid lines, and text labels are styled using theme token classes like `fill-foreground`, `fill-muted-foreground`, and `stroke-border`, making them adapt to light and dark modes.

### Preset Variations

- **Glassmorphic Area & Fills**: Employs frosted glass saturation with high-intensity backdrop blur, semi-transparent borders, and HSL transparency filters.
- **Theme-Aware Radial Track**: Uses `var(--muted)` background fills for radial bar tracks so that the chart indicators remain visible against the card background on both light and dark backgrounds.
