...
<ComponentPreview sandbox="uploader">
...
const uploaderVariants = tv({
  ...
  variants: {
    variant: {
      ...
      glass: 'bg-white/5 dark:bg-black/20 border-white/20 dark:border-white/10 backdrop-blur-md shadow-lg',
...
Explore all interactive states and combinations of the Uploader component.

### 1. Preset Variations

Every variation supports our 4 built-in theme presets.
...
export default function GlassUploaderDemo() {
  return <Uploader variant="glass" />
}
```

</ComponentPreview>

#### Retro Neobrutalism Preset
...
export default function RetroUploaderDemo() {
  return <Uploader variant="retro" />
}
```
...
export default function GlowUploaderDemo() {
  return <Uploader variant="glow" />
}
```
...
Vibe UI's Uploader is a stateful dropzone interface built on top of native file inputs. It provides a visual area for drag-and-drop selections, file validators, size validation indicators, and reactive layout grids.

### Accessibility

- **Keyboard Focusable:** Fully clickable and focusable dropzone targets.
- **Screen Reader Support:** Text labels explicitly describing size limitations.
