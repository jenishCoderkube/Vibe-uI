  ...
  return (
    <div className="w-full max-w-xl py-6 select-none text-center">
      <p className="text-xs text-muted-foreground mb-4 font-medium flex items-center justify-center gap-1.5">
        <Keyboard className="h-3.5 w-3.5 text-primary" /> Click below or press{' '}
        <kbd className="bg-muted px-1.5 py-0.5 rounded border border-border text-[10px] font-mono">
          ⌘K
        </kbd>{' '}
        /{' '}
        <kbd className="bg-muted px-1.5 py-0.5 rounded border border-border text-[10px] font-mono">
      ...
      <CommandDialog open={open} onOpenChange={setOpen} variant="default">
        <CommandInput placeholder="Search workspace files & tasks..." />
        <CommandList>
          <CommandGroup heading="Quick Actions">
            <CommandItem
              onSelect={() => {
                alert('Launching Terminal...')
                setOpen(false)
              }}
            >
